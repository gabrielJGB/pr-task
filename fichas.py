...
# link = "https://www.promiedos.com.ar/ficha=xjktktrdqcvcv"
...
def get_info_equipo(trs,equipo):
    seccion = 0
    info_equipo = {"titulares":[],"suplentes":[],"tecnico":"","goles":"","amarillas":[],"rojas":[],"cambios":[],"incidencias":[]}
    ...
    def handle_incidencia(tr,tipo):
        ...
        for elem in arr:
            ...
            incidencia = {"minuto":handle_minuto(incidencia_elem[0].strip()),"jugador":incidencia_elem[1].strip().split("⇆"),"tipo":tipo,"equipo":equipo}
...
def crawl_ficha(ficha,pid):
    ...
    ficha_partido = {"local":local,"visitante":visitante,"id_ficha":ficha,"id_partido":pid,"info_local":info_local, "info_visitante": info_visitante,"escudo_local":escudo_local,"escudo_visitante":escudo_visitante,"estado":estado,"goles_local":goles_local,"goles_visitante":goles_visitante,"arbitro":arbitro,"incidencias":sorted(get_incidencias_partido(),key=lambda x: x["minuto"])}
...
if res.status_code == 200:
    ...
    partidos_res = [{"ficha":partido["ficha"],"id":partido["id"],"estado":partido["estado"]} for fecha in fechas_resp["fechas"] for partido in fecha["partidos"]]
    ...
    if partido_res["ficha"] != "" and partido_res["estado"] == "finalizado":        
        for partido_verificar in partidos_verificar:
            if partido_verificar["id"] == partido_res["id"] and partido_verificar["verificado"] == False:
                ficha_partido = crawl_ficha(partido_res["ficha"],partido_res["id"])
